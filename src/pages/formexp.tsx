...
interface MyProps {
  ...
  name: string;
  // rest: HTMLAttributes<HTMLInputElement> | HTMLAttributes<HTMLInputElement>[];
}

const Input: React.FC<MyProps> = ({ register, name }) => {
  return (
    <input
      {...register(name)}
      className="w-[60%] rounded-xl bg-blue-300 px-3 py-3 shadow shadow-gray-400/100"
    />
  );
};
